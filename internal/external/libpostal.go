...
func ExtractWithLibpostal(raw string) LP {
	...
	for _, c := range comps {
		switch c.Label {
		case "house_number": lp.House = c.Value
		case "road":         lp.Road = c.Value
		case "unit":         lp.Unit = c.Value
		case "level":        lp.Level = c.Value
		case "suburb":       lp.Ward = c.Value
		case "city":         lp.City = c.Value
		case "state":        lp.Province = c.Value
		}
		covered += len(strings.Fields(c.Value))
	}
	if total > 0 { lp.Coverage = float64(covered)/float64(total) }
	return lp
}