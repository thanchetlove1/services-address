...
func main() {
	...
	// Setup normalizer
	normalizer := normalizer.NewTextNormalizerV2()
	if err := normalizer.LoadRules(); err != nil {
		log.Fatal("Lỗi load normalizer rules:", err)
	}
	...
	// Setup service
	addressService := services.NewAddressService(parser, normalizer, searcher, logger)
	...
	testAddresses := []string{
		...
		"456/78 Lê Lợi, Phường Đông Khê, Quận Ngô Quyền, Thành phố Hải Phòng 0987654321",
		"789 Nguyễn Trãi Ward 5 District 5 Ho Chi Minh City Vietnam +84-901-234-567",
	}
	...
	for i, addr := range testAddresses[:2] { // Test 2 addresses only
		...
		normResult := normalizer.NormalizeAddress(addr)
		if normResult != nil {
			fmt.Printf("✅ Normalized: %s\n", normResult.NormalizedNoDiacritics)
			fmt.Printf("📍 Fingerprint: %s\n", normResult.Fingerprint)
			if normResult.POI.Company != "" {
				fmt.Printf("🏢 POI Company: %s\n", normResult.POI.Company)
			}
			if normResult.POI.Building != "" {
				fmt.Printf("🏗️  POI Building: %s\n", normResult.POI.Building)
			}
		} else {
...
func printResult(result *models.AddressResult) {
	...
	if result.Components.Street != nil && result.Components.Street.Name != nil {
